...
inline constexpr double deg2rad(double x) { return x * pi() / 180; }

inline constexpr double rad2deg(double x) { return x * 180 / pi(); }

inline constexpr double mph2mps(double x) { return x / 2.2369; }

inline constexpr double mps2mph(double x) { return x * 2.2369; }
...
inline std::vector<double> global2car(std::vector<double> pos,
...
// ref is car's position with global yaw angle (rad)
inline std::vector<double> car2global(std::vector<double> pos,
                                      std::vector<double> &ref) {
  double cos_yaw = cos(ref[2]);
  double sin_yaw = sin(ref[2]);
